...
void Exit ( int x ) {
    ...
    // Define the sigaction struct for handling signals
    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));
    
    if ( x == SIGSEGV ){
        sa.sa_handler = default_SIGSEGV_handler; // Set the custom handler
        sigaction(SIGSEGV, &sa, nullptr);        // Install the handler for SIGSEGV
        // sigaction(SIGSEGV, default_SIGSEGV_handler) ;
    }
    else{
        sa.sa_handler = default_SIGTSTP_handler; // Set the custom handler
        sigaction(SIGTSTP, &sa, nullptr);        // Install the handler for SIGTSTP
        // sigaction(SIGTSTP, default_SIGTSTP_handler) ;
    }
    raise(x) ; // Re-raise the signal
}
